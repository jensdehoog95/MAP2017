...
\subsubsection{Solutions} The general approach is to modulate the ESC-signal. More specifically, by switching the ESC on and off at a particular frequency, the speed can be regulated. The switching signal can be compared with a gate which allows the motorsignal to pass through.

The first concept dealt with a PWM-signal which would modulate the ESC-controlsignal. The motor controller was turned on and off using the generated square wave. By varying the duty cycle of that wave, the active time of the ESC varied with it.
...
\begin{figure}
    ...
    \centering
    \caption{a) Shows the motorsignal in its neutral state. b) Shows the motorsignal when the car begins to drive. Note that these figures are zoomed in order to display the difference between the two signals.}
    \label{fig:rgb2hsv}
\end{figure}

%% ACTUAL IMPLEMENTATION TOEVOEGEN %%

Although this solution should work in theory, it appeared to be more difficult in practice because of limitations of the hardware components. First, the ESC does not handle the motorsignal, which is switched by the gate, very well. The controller does not accept the signal at a high switching frequency. Besides that, the Teensy board cannot generate signals with a low switching rate. 
%%% EXPLANATIONS TOEVOEGEN %%%

The second concept takes a different approach. Instead of generating a switching signal 
