...
\subsubsection{Solution} The general approach is to modulate the ESC-signal. More specifically, by switching the ESC on and off at a particular frequency, the speed can be regulated. The switching signal can be compared with a gate which allows the motorsignal to pass through.
...
Although this solution should work in theory, it appeared to be more difficult in practice because of limitations of the hardware components. First, the ESC does not handle the motorsignal, which is switched by the gate, very well. The controller does not accept the signal at a high switching frequency. Also, the Teensy board cannot generate signals at a low switching rate. 
%%% NAKIJKEN %%%
