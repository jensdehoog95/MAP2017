...
\subsubsection{Problem}A problem arises when the ESC-signal has a duty cycle which is a fraction higher than the neutral value. The motor does not drive instantly. Only at a particular numeric value of 10120 or at a duty cycle of 15.44189\%, the car starts driving at walking speed. This speed is too fast when 
developing and debugging software. Thus, a certain deadzone exists between duty cycles 15\% and 15.44189\% which cannot be filled in by the hardware components. A software approach is needed in order to drive more slowly.

\subsubsection{Solution} The general approach is to modulate the ESC-signal. By turning the ESC on and off frequently, 
%The first approach was to modulate the ESC-signal with another PWM-signal. A square wave with a variable duty cycle would be generated, which would switch 
